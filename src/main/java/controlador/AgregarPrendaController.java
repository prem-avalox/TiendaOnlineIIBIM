...
public class AgregarPrendaController extends HttpServlet {
	...
	private void guardar(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// Procesar Stock
			List<StockTalla> stockList = new ArrayList<>();
			if (tallasArr != null && cantidadesArr != null) {
				for (int i = 0; i < tallasArr.length; i++) {
					int cantidad = Integer.parseInt(cantidadesArr[i]);
					if (cantidad > 0) { // Solo agregar si hay stock
						StockTalla st = new StockTalla(cantidad, Talla.valueOf(tallasArr[i]));
						st.setPrenda(p);
						stockList.add(st);
					}
				}
			}
			p.setStockTallas(stockList);
